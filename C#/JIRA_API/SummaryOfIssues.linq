...
void Main() //assignee = currentUser() OR component = PAPA  and sprint in("All")
{
	...
	//String[] foos = new String[] {"assignee = currentUser() AND resolution = Unresolved and sprint in (openSprints()) ORDER BY updatedDate DESC"}; 
	String[] foos = new String[] {"assignee = currentUser() AND resolution = Unresolved and sprint in (openSprints()) ORDER BY updatedDate DESC"}; 
	//assignee = currentUser() AND resolution = Unresolved and sprint in (closedSprints()) ORDER BY updatedDate DESC
...
public class JiraManager
{
	...
	public List<Issue> GetIssues(
		...
		int maxResult = 80)
	{
		fields = fields ?? new List<string> { "summary"  };
