		...
		foreach (var element in findResults)
		{
			try
			{
				...
				//SaveEmailAttachment(service,element.Id);
				 string myAttachment=GetAttachmentsFromEmail(service,element.Id);
...
public static void PostAttachmentJira(string subject, string body) //http://stackoverflow.com/questions/13974208/creating-jira-issue-via-rest-c-sharp-httpclient
{ //https://answers.atlassian.com/questions/79902/using-httpclient-c-to-create-a-jira-issue-via-rest-generates-bad-request-response
  // string data = @"{""fields"":{""project"":{""key"": ""TEST""},""summary"": ""REST EXAMPLE"",""description"": ""Creating an issue via REST API"",""issuetype"": {""name"": ""Bug""}}}";
...
public static string GetAttachmentsFromEmail(ExchangeService service, ItemId itemId) //https://msdn.microsoft.com/en-us/library/office/dn726695(v=exchg.150).aspx
{
    ...
    foreach (Attachment attachment in message.Attachments)
    {
        if (attachment is FileAttachment)
        {
			...
			return "C:\\temp\\" + fileAttachment.Name;
        }
