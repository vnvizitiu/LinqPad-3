...
class Program //http://stackoverflow.com/questions/17107220/convert-dataset-to-list
{
			...
			{
				{
					...
					retObjts objEmp = new retObjts();// create the object of class retObjts 
					List<retObjts> empList = new List<retObjts>();
					...
					for (int i = 1; i < table; i++)// set the table value in list one by one
					{
						...
						{
							empList.Add(new retObjts { Name = Convert.ToString(dr["Tables"]), Type=DS.Tables[i].TableName });
						}
					}
	...
	public static DataSet getObjc()
	{
		...
		try
		{
			...
			SqlDataAdapter viewAdapter = new SqlDataAdapter(
			  "SELECT TABLE_NAME as Tables FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'VIEW'", conn);
			SqlDataAdapter functionAdapter = new SqlDataAdapter(
			  "SELECT name as Tables FROM sys.objects WHERE type_desc LIKE '%FUNCTION%'", conn);
			SqlDataAdapter procAdapter = new SqlDataAdapter(
			  "select ROUTINE_NAME as Tables from Information_schema.routines where routine_type = 'PROCEDURE'", conn);
