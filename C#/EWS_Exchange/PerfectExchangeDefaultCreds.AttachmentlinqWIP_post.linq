	...
	//	FindItemsResults<Item> findResults = service.FindItems(WellKnownFolderName.Inbox, sf, itemView);
		...
		foreach (var element in findResults)
		{
			try
			{
				...
				string JiraResponse=RaiseJira(element.Subject,element.Body);
				
				string postUrl = "https://jira.odp.cba/rest/api/2/issue/"+ JiraResponse+"/attachments";
...
public static string RaiseJira(string subject, string body) //http://stackoverflow.com/questions/13974208/creating-jira-issue-via-rest-c-sharp-httpclient
{ //https://answers.atlassian.com/questions/79902/using-httpclient-c-to-create-a-jira-issue-via-rest-generates-bad-request-response
  ...
  //	string path = @"C:\Download_Codes\JiraExample_Console\JiraExample\JSON_Results\RaiseIssue.json";
	...
	string jDataOld = @"{""fields"":{""project"":{""key"": ""BIDA""},""summary"": ""Jira raised via REST API-alternative"",
                               ""description"": ""To test if raising JI?A via REST API is possible"",""issuetype"": {""name"": ""Improvement""}}}";

	string jData = @"{""fields"":{""project"":{""key"": ""BIDA""},""summary"": ""Jira raised via Exchange MAPI with Attachments"",
                               ""description"": ""Success will have attachments with assignee and reporter"",  ""assignee"": { ""name"": ""samtran"" },""reporter"": { ""name"": ""samtran"" }
	...
	#region private
	byte[] cred = UTF8Encoding.UTF8.GetBytes("samtran:Sydney123");
	#endregion
	...
	if (response.IsSuccessStatusCode)
	{
		...
		var sData = jss.Deserialize<Dictionary<string, string>>(result);
...
public static void PostFile(string restUrl,IEnumerable<FileInfo> filePaths)
        {
