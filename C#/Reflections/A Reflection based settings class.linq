	...
	static void Main(string[] args)
	{
		...
		else
		{
			...
			foreach (PropertyInfo propertyInfo in properties)
			{
				...
				person.SetProperty(propertyInfo, 18);
			}
			person.Save();
			Console.WriteLine("Thank you! I have saved your information for next time.");
		}
		Console.WriteLine("See added info in the file created above");
	}
}
...
{//http://csharp.net-tutorials.com/reflection/reflection-based-settings-class/
	...
	public void Load()
	{
		if (File.Exists(@"C:\LinqPad_Queries\C#\Reflections\settings.dat"))
		{
			...
			string[] settings = File.ReadAllLines(@"C:\LinqPad_Queries\C#\Reflections\settings.dat");
	...
	public void Save()
	{
		Type type = this.GetType();
		PropertyInfo[] properties = type.GetProperties();
		TextWriter tw = new StreamWriter(@"C:\LinqPad_Queries\C#\Reflections\settings.dat");
		foreach (PropertyInfo propertyInfo in properties)
		{
